    ...
    >
      <div className="my-8 w-full max-w-2xl rounded-xl border border-border bg-white p-6 shadow-xl" onClick={(event) => event.stopPropagation()}>
        <div className="flex items-start justify-between gap-3 border-b border-border pb-4">
          <div>
            <h3 className="text-xl font-semibold text-foreground">{personName(patient, t('common.unnamedPatient'))}</h3>
            <p className="text-sm text-foreground/60">{t('overview.patientIdLine', { id: patient.id })}</p>
          </div>
          <button type="button" onClick={onClose} className="rounded-md p-1 text-foreground/60 hover:bg-secondary hover:text-foreground" aria-label={t('common.close')}>
            <X size={18} />
          </button>
        </div>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <div className="rounded-lg border border-border bg-secondary/30 p-3">
                <p className="text-xs text-foreground/60">{t('overview.reports')}</p>
                <p className="text-2xl font-semibold text-foreground tabular-nums">{reports.length}</p>
              </div>
              <div className="rounded-lg border border-border bg-secondary/30 p-3">
                <p className="text-xs text-foreground/60">{t('overview.meanQuality')}</p>
            ...
            {distribution.length ? (
              ...
              <p className="text-sm text-foreground/60">{t('overview.noReportYet')}</p>
            )}
          </div>

          <div>
            <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-foreground/60">{t('overview.reportHistory')}</p>
            <div className="max-h-72 space-y-2 overflow-y-auto">
                ...
                .map((report) => (
                  ...
                  >
                    ...
                    <span className="flex items-center gap-2">
                      <span className={`rounded-md border px-2 py-0.5 text-xs font-semibold ${TONE_BADGE[classMeta(report.assessment, language).tone]}`}>
                        {classMeta(report.assessment, language).label}
                      </span>
              ...
              {!reports.length ? <p className="text-sm text-foreground/60">{t('overview.nothingToShow')}</p> : null}
            </div>
          </div>
        </div>
      </div>
